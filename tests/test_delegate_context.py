...
# demoai 事故复刻：长报错粘贴，且含足以触发旧规则委派的复杂度词
ERR_INPUT = (
    "Spring Boot 启动失败，所有服务都起不来，先检查数据库再部署。\n"
    "Caused by: java.sql.SQLSyntaxErrorException: "
    "Table 'demoai.user' doesn't exist\n"
...
DEBUG_MESSAGES = [
    ...
    {"role": "user", "content": "demoai 项目启动还是有问题"},
    {"role": "assistant", "content": "我检查了 demoai 的配置，数据库连接指向 localhost。"},
]
...
class TestBuildContextBrief:
    def test_extracts_goal_recent_messages_and_chinese_path(self):
        ...
        assert "当前任务目标：demoai 项目启动还是有问题" in brief
...
class TestDebuggingContinuationGate:
    ...
    def test_non_error_input_not_gated(self):
        """非报错类输入不触发闸门，走原有三支规则。"""
        agent = _bare_agent(DEBUG_MESSAGES)
        assert agent._should_delegate("把 demoai 的所有模块分别做全面分析并部署") is True
        assert agent._should_delegate("demoai 的 readme 帮我看下") is False

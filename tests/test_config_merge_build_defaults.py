...
class TestMergeBuildDefaults:
    def test_new_keys_merged(self, merge_env):
        ...
        _write(tmpl_path, {
            "api_keys": {},
            "update_manifest_url": "http://172.16.100.81:8080/release/version.json",
            "ui_theme": {"app_name": "中新社智能体中心"},
        })
        cfg = cfgmod.load_config()
        assert cfg["update_manifest_url"] == "http://172.16.100.81:8080/release/version.json"
        assert cfg["ui_theme"]["app_name"] == "中新社智能体中心"
        # 用户原有键不受影响
    ...
    def test_user_app_name_preserved(self, merge_env):
        ...
        _write(tmpl_path, {"ui_theme": {"app_name": "中新社智能体中心"}})
        cfg = cfgmod.load_config()
    ...
    def test_manifest_url_follows_template(self, merge_env):
        ...
        _write(tmpl_path, {"update_manifest_url": "http://172.16.100.81:8080/release/version.json"})
        cfg = cfgmod.load_config()
        assert cfg["update_manifest_url"] == "http://172.16.100.81:8080/release/version.json"

# -*- coding: utf-8 -*-
"""沙箱治理（一期）API：顶层条目浏览、手动归类、手动删除、一键清 tmp、任务交付物。
...
一期范围：不做自动 TTL 清理、水位告警、归档压缩、存量强制搬迁。
...
按条目数间隔检查，超时返回 partial=True 的部分统计。
...
def _deliverable_entries(root: str, outputs_dir: str) -> list:
    """把 outputs/ 展开一层：每个 task_<id>/ 子目录作为一条 deliverable 条目。
    子目录若是逃逸链接（junction/symlink 指向沙箱外）则跳过并记日志。"""
    out = []
    try:
...
async def list_sandbox_entries():
    """sandbox 顶层条目列表（outputs/ 展开为 task_<id> 交付物条目）。
    ...
    known = [e["size"] for e in entries if e.get("size") is not None]
...
async def move_sandbox_entry(req: SandboxMoveRequest):
    """归类移动顶层条目到 projects/ 或 tmp/；目标重名返回 409。"""
    dest = (req.dest or "").strip()
    if dest not in ("projects", "tmp"):
        raise HTTPException(status_code=400, detail="dest 仅允许 projects 或 tmp")
    joined, name = _resolve_top_level(req.path)
    if name in _FORBIDDEN_NAMES:

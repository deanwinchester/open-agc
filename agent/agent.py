...
class OpenAGCAgent:
    ...
    def __init__(self, model: str = "gpt-4o", session_id: Optional[int] = None,
        ...
        _prompt_exec = (
            ...
            f"\n## 沙箱分区\n"
            f"沙箱工作目录（{{cwd_dir}}）按用途分区存放，必须按区放置，"
            f"严禁在根目录直接新建文件或目录：\n"
            f"- projects/：长期项目（代码仓库、持续维护的项目），不会被自动清理\n"
            f"- outputs/<主题语义名>/：本任务的交付物（最终成果文件），目录用能概括交付主题的语义名"
            f"（如 outputs/唐嫣照片/、outputs/月度报告/），与大任务检查点的 files_dir 口径保持一致；"
            f"动笔前先 ls outputs/ 查看已有目录——与本次任务同主题的直接复用同一目录，"
            f"不同主题撞名时加数字后缀（如 报告-2/）；实在无法命名才用 outputs/task_<当前任务ID>/ 兜底"
            f"（当前任务 ID 见「当前任务检查点」段）\n"
            f"- tmp/：一次性脚本、中间产物等临时文件\n"
            f"- downloads/：安装包等大文件（queue_download 工具下载的文件由下载管理器单独管理，不在此目录）\n"
            f"\n## Python 后台进程\n"
            ...
            f"5. SSH 远程主机一律用 execute_python+paramiko（本机私钥或 {{{{secret:名称.*}}}} 凭据），\n"
            f"   不要用 execute_shell+ssh——交互式密码/主机确认会把命令挂死；\n"
            f"   ssh 挂起≠密码错误，先试 paramiko（含已有凭据）再考虑 request_secret。\n"
            f"\n{{system_env}}\n"
            f"\n# 项目创建规范\n"
        ...
        _prompt_mech = (
            ...
            f"（先搜索「插件」启用工具，开发完成后调用 POST /api/plugins/scan 热更新，"
            f"并用 develop_plugin(action=\"verify\") 验收通过后再交付，无需重启服务）。"
